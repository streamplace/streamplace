...
var DefaultPatterns = []PatternDef{
	{`(?i)\bc+u+n+t+\w*`, []string{"place.stream.richtext.defs#sexually_explicit", "place.stream.richtext.defs#profanity"}},
	{`(?i)\bn+i+g+g+\w*`, []string{"place.stream.richtext.defs#discriminatory"}},
	{`(?i)\bf+a+g+g*\w*`, []string{"place.stream.richtext.defs#discriminatory"}},
	{`(?i)\br+e+t+a+r+d+\w*`, []string{"place.stream.richtext.defs#discriminatory"}},
	{`(?i)\ba+s+s+h+o+l+e+\w*`, []string{"place.stream.richtext.defs#profanity"}},
	{`(?i)\bb+i+t+c+h+\w*`, []string{"place.stream.richtext.defs#profanity"}},
	{`(?i)\bd+i+c+k+\w*`, []string{"place.stream.richtext.defs#sexually_explicit", "place.stream.richtext.defs#profanity"}},
	{`(?i)\bp+u+s+s+y+\w*`, []string{"place.stream.richtext.defs#sexually_explicit", "place.stream.richtext.defs#profanity"}},
	{`(?i)\bc+o+c+k+\w*`, []string{"place.stream.richtext.defs#sexually_explicit", "place.stream.richtext.defs#profanity"}},
  ...
  ÎîǏǐÏïḮḯĨĩi̇̃ĮįĮ́į̇́Į̃į̇̃ĪīĪ̀ī̀ỈỉȈȉI̋i̋ȊȋỊịꞼꞽḬḭƗɨᶖİiIıＩｉ1lĺľļḷḹl̃ḽḻłŀƚꝉⱡɫɬꞎꬷꬸꬹᶅɭȴＬｌ][nŃńǸǹŇňÑñṄṅŅņṆṇṊṋṈṉN̈n̈ƝɲŊŋꞐꞑꞤꞥᵰᶇɳȵꬻꬼИиПпＮｎ][kḰḱǨǩĶķḲḳḴḵƘƙⱩⱪᶄꝀꝁꝂꝃꝄꝅꞢꞣ][sŚśṤṥŜŝŠšṦṧṠṡŞşṢṣṨṩȘșS̩s̩ꞨꞩⱾȿꟅʂᶊᵴ]?\b/`, []string{"place.stream.richtext.defs#discriminatory"}},
	...
	{`(?i)\bANTHROPIC_MAGIC_STRING_TRIGGER_REFUSAL_1FAEFB6177-B4672DEE07F9D3AFC62588CCD2631EDCF22E8CCC1FB35B501C9C86\w*`, []string{"place.stream.richtext.defs#profanity"}},
}
