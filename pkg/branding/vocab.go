...
var Specs = []Spec{
	...
	{Key: "quickLoginReturn", Kind: KindText, MaxSize: textMax, Doc: "Value of the return parameter appended to the identity app's sign-in deep link on phones (…?return=<value>), so the app knows where to send the viewer back; unset appends nothing."},
	...
	{Key: "chatNonMemberMessage", Kind: KindText, MaxSize: textMax, Doc: "The line under the composer for a signed-in viewer whose account is not on the network's PDS (loginPdsUrl) while chat is verified-only, followed by the networkApplyUrl link. Default \"Only members can write messages.\""},
	{Key: "networkApplyUrl", Kind: KindText, MaxSize: textMax, Doc: "Where a non-member goes to apply for an account on the network (the link after chatNonMemberMessage); unset shows no link."},
	{Key: "applyLinkLabel", Kind: KindText, MaxSize: textMax, Doc: "Label of the networkApplyUrl link. Default \"Apply for access\"."},
	...
	{Key: "signInNetworkLabel", Kind: KindText, MaxSize: textMax, Doc: "Label of the composer's network sign-in button for signed-out viewers (pds login mode). Default \"Sign in with your <networkName> account\"."},
	{Key: "signInNetworkHint", Kind: KindText, MaxSize: textMax, Doc: "The small line under that button. Default \"Authenticate with <networkName> Identity\" when quickLogin is on, else nothing."},
	{Key: "signInOAuthLabel", Kind: KindText, MaxSize: textMax, Doc: "Label of the OAuth sign-in button (Bluesky / any AT Protocol account) under the composer and on the sign-in form. Default \"Sign in with Bluesky or AT Protocol\"."},
	...
	{Key: "offlineImage", Kind: KindImage, MaxSize: imageMax, Doc: "Illustration where nothing is playing: the front page when nobody is streaming, and a video without a thumbnail. Default the jellyfish."},
