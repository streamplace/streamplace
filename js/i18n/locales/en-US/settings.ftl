...
branding-bundle-description = A bundle is a zip with branding.yaml (everything set here, with the defaults listed for reference) plus the image files. Unzipped, it is a brand directory: edit the YAML by hand, import it on another node, or build branded apps from it (SP_BRAND_DIR).
...
branding-domains = Custom domains
branding-domains-description = Hostnames this node serves under their owner's brand instead of its own. The brand is the owner's place.stream.branding.brand record for the hostname; editing it here publishes it to their account.
branding-domains-synced = Owner { $owner }, following their brand record
branding-domains-no-record = Owner { $owner }, no brand published yet
branding-domains-sync-error = Last sync failed: { $error }
branding-domains-edit = Edit brand
branding-domains-editing = Editing
branding-domains-sync = Sync now
branding-domains-sync-failed = Sync failed
branding-domains-remove = Remove
branding-domains-remove-failed = Could not remove the domain
branding-domains-add = Add a custom domain
branding-domains-owner-placeholder = Owner handle or DID (default: you)
branding-domains-add-button = Add domain
branding-domains-add-help = Point the domain's DNS at this node first. The owner can then brand it from here, or publish the record from any atproto client.
branding-domains-add-failed = Could not add the domain
branding-bundle-export-failed = Export failed
branding-bundle-invalid = That bundle can't be imported
branding-navigation = Navigation
