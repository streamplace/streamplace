...
func runE2E(ctx context.Context, devEnvPath string) error {
	...
	password := "test"
	out, err := indigoatproto.ServerCreateAccount(ctx, xrpcc, &indigoatproto.ServerCreateAccount_Input{
		Handle:   handle,
		Email:    &email,
		Password: &password,
	})
	...
	session, err := indigoatproto.ServerCreateSession(ctx, xrpcc, &indigoatproto.ServerCreateSession_Input{
