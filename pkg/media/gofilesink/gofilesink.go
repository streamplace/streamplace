...
func (f *FileSink) Start(self *base.GstBaseSink) bool {
	if f.state.started {
		self.ErrorMessage(gst.DomainResource, gst.ResourceErrorSettings, "GoFileSink is already started", "GoFileSink is already started")
		return false
	}

	if f.settings.location == "" {
		self.ErrorMessage(gst.DomainResource, gst.ResourceErrorSettings, "No location configured on the filesink", "No location configured on the filesink")
		return false
	}
	...
	if err != nil {
		self.ErrorMessage(gst.DomainResource, gst.ResourceErrorOpenWrite,
			fmt.Sprintf("Could not open %s for writing", f.settings.location), err.Error())
		return false
	}
	self.Log(CAT, gst.LevelDebug, fmt.Sprintf("Opened file %s for writing", f.settings.location))
