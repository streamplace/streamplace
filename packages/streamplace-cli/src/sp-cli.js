...
import {
  ...
  commandServe,
  commandBuild
} from "./command/commandActions";
import * as actionNames from "./constants/actionNames";

const configDefault = resolve(
  process.env.HOME,
  ".streamplace",
  "sp-config.yaml"
);
...
export default program
  ...
  .option(
    "--sp-config <file>",
    "location of sp-config.yaml (default $HOME/.streamplace/sp-config.yaml)",
    configDefault
  );
...
program
  ...
  .option(
    "--docker-prefix <name>",
    "prefix of built Docker images (default streamplace)",
    "streamplace"
  )
  .option(
    "--docker-tag <name>",
    "tag of built Docker images (default latest)",
    "latest"
  )
  .option("--dev-server <url>", "url of your development server")
  .action(function(command) {
