...
// A pre-live token keeps every URL on the node: the CDN must not see (or
// cache) a streamer's private preview.
func TestLiveTokenKeepsURLsSelfHosted(t *testing.T) {
	l := liveCDN{URL: "https://live.b-cdn.net", TTL: time.Minute}
	u := l.segmentURL("did:plc:abc", "1", "42", "", "tok")
	require.True(t, strings.HasPrefix(u, "/xrpc/place.stream.playback.getLiveSegment?"))
	require.Contains(t, u, "token=tok")
	require.Contains(t, liveTrackPlaylistURL("did:plc:abc", "1", "", false, "tok"), "token=tok")
}
