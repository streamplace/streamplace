...
try {
  ...
  const results = spawnSync("kubectl", [
    "get",
    "secret",
    "-o",
    "json",
...
export default class ServerAuth extends Resource {
  ...
  authUpdate(ctx, doc, newDoc) {
    throw new Resource.APIError(
      403,
      "You may not modify existing ServerAuths."
    );
  }
  ...
  authCreate(ctx, doc) {
    if (!ctx.user || ctx.user.id !== doc.userId) {
      throw new Resource.APIError(
        403,
        "You may only make ServerAuths for yourself"
      );
    }
  }
  ...
  authDelete(ctx, doc) {
    if (!ctx.user || ctx.user.id !== doc.userId) {
      throw new Resource.APIError(
        403,
        "You may only delete your own ServerAuths."
      );
    }
  }
