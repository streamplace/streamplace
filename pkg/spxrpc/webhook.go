...
// validateWebhookURL resolves the hostname of a URL and checks if any of its
// IPs are public, returning an error if all are private, loopback, or invalid.
func validateWebhookURL(rawURL string) error {
	...
	// 3. Check if at least one IP is a valid, public address
	for _, ip := range ips {
		if aqhttp.IsIPAllowed(ip) {
			// Found a valid public IP, so the URL is acceptable.
			return nil
		}
	}

	// 4. If the loop completes, no public IPs were found.
	return fmt.Errorf("host '%s' resolves to a private or invalid IP address", hostname)
}
