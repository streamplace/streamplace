...
flows, and tears the harness down. `SP_E2E_BIN` overrides which binary it
runs (default: `build-<os>-<arch>/streamplace`). On a host without the cgo
runtime, run it inside the build container.
...
The `e2e-web` job in `.github/workflows/build.yaml` runs
`make linux-amd64 && make e2e-web` inside the builder container: it builds
the full binary (which embeds the web app), installs Playwright chromium,
and drives `hack/e2e-web-local.sh`. On failure, the harness log tail is
printed to the job output and `playwright-report/` + `test-results/` +
`harness.log` are uploaded as a workflow artifact.
