...
function parseRgbString(rgbString: string): place.stream.chat.profile.Color {
  if (
    !rgbString ||
    (!rgbString.startsWith("rgb(") && !rgbString.startsWith("rgba(")) // eslint-disable-line streamplace/no-token-literals -- rgb() string parsing
  ) {
...
export function useNameColorPicker() {
  ...
  const [tempColor, setTempColor] = useState("#bd6e86"); // eslint-disable-line streamplace/no-token-literals -- default name color
