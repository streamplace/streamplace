// Too many dependencies. Disabled 2018-01-26. Would be nice to bring back.
...
//     return SK.broadcasts
//       .create({ title: testBroadcastTitle })
...
//     return SK.broadcasts
//       .update(testBroadcastId, { title: testBroadcastTitle })
//       .then(function(broadcast) {
//         expect(broadcast).to.be.ok();
//         expect(broadcast.title).to.be(testBroadcastTitle);
//       })
...
//       const doCreatePromise = SK.broadcasts.create({
//         title: watchBroadcastTitle
//       });
//       return Promise.all([createdEventPromise, doCreatePromise]).then(function(
//         [[newDocs, createdIds], newDoc]
//       ) {
...
//       const doUpdatePromise = SK.broadcasts.update(watchBroadcastId, {
//         title: watchBroadcastTitle
//       });
//       return Promise.all([updateEventPromise, doUpdatePromise]).then(function(
//         [[newDocs, updatedIds], changedDoc]
//       ) {
//         expect(updatedIds).to.eql([watchBroadcastId]);
//         expect(newDocs.filter(doc => doc.id === watchBroadcastId)).to.eql([
//           changedDoc
//         ]);
//       });
//     });
...
//       return Promise.all([deleteEventPromise, doDeletePromise]).then(function(
//         [[newDocs, removedIds]]
//       ) {
