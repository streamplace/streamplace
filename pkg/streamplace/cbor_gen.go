...
func (t *SchemaFile) MarshalCBOR(w io.Writer) error {
	if t == nil {
		_, err := w.Write(cbg.CborNull)
		return err
	}
	...
	// t.Defs (map[string]lexicon.SchemaDef) (map)
	if len("Defs") > 1000000 {
		return xerrors.Errorf("Value in field \"Defs\" was too long")
	}

	if err := cw.WriteMajorTypeHeader(cbg.MajTextString, uint64(len("Defs"))); err != nil {
		return err
	}
	if _, err := cw.WriteString(string("Defs")); err != nil {
		return err
	}

	{
		if len(t.Defs) > 4096 {
			return xerrors.Errorf("cannot marshal t.Defs map too large")
		}

		if err := cw.WriteMajorTypeHeader(cbg.MajMap, uint64(len(t.Defs))); err != nil {
			return err
		}
...
func (t *SchemaFile) UnmarshalCBOR(r io.Reader) (err error) {
	*t = SchemaFile{}
	...
	if extra > cbg.MaxLength {
		return fmt.Errorf("SchemaFile: map struct too large (%d)", extra)
	}
	...
	for i := uint64(0); i < n; i++ {
		...
		case "Defs":
			...
			if extra > 4096 {
				return fmt.Errorf("t.Defs: map too large")
			}

			t.Defs = make(map[string]SchemaDef, extra)
